...
export const sampleQuadratic =
    (points: ReadonlyVec[], opts?: number | Partial<SamplingOpts>) => {
        if (isPlainObject(opts) && (<SamplingOpts>opts).dist !== undefined) {
            return new Sampler(sampleQuadratic(points, (<SamplingOpts>opts).num || DEFAULT_SAMPLES))
                .sampleUniform((<SamplingOpts>opts).dist, (<SamplingOpts>opts).last !== false);
        }
        opts = isNumber(opts) ?
            {
                num: opts,
