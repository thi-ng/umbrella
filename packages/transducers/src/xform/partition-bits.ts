 ...
 */
export function partitionBits(destSize: number, srcSize?: number): Transducer<number, number>;
...
const small = ([init, complete, reduce]: Reducer<any, number>, n: number, wordSize: number): Reducer<any, number> => {
...
const large = ([init, complete, reduce]: Reducer<any, number>, n: number, wordSize: number): Reducer<any, number> => {
    const m1 = (1 << wordSize) - 1;
    let r = 0;
    let y = 0;
    return [
