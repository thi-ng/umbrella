...
group("defmulti", {
    flatten: () => {
        ...
        assert.ok(
            flatten.add(
                DEFAULT,
                (x, acc: any[]) => (acc.push(x.toString()), acc)
            )
        );
    ...
    sexpr: () => {
        const exec = defmulti<number>((x) =>
            Array.isArray(x) ? x[0] : typeof x
        );
        assert.ok(
            exec.add("+", ([_, ...args]) =>
    ...
    apr: () => {
        const apr = defmulti<any, number>(
            ({ type, balance }) =>
                `${type}-${
                    balance < 1e4 ? "low" : balance < 5e4 ? "med" : "high"
                }`,
            {},
            {
