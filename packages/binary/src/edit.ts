 ...
 */
export const bitClear = (x: number, bit: Bit) =>
    (x & ~(1 << bit)) >>> 0;
 ...
 * @param bit
 */
export const bitFlip = (x: number, bit: Bit) =>
    (x ^ (1 << bit)) >>> 0;
 ...
 * @param bit bit number (0..31)
 */
export const bitSet = (x: number, bit: Bit) =>
    (x | (1 << bit)) >>> 0;

export const bitSetWindow = (x: number, y: number, from: number, to: number) => {
