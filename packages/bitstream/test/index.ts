...
describe("BitOutputStream", function () {
    it("write", () => {
        ...
        assert. deepStrictEqual([...o.write(0xff, 1).buffer], [0x80, 0, 0, 0, 0, 0, 0, 0], "1");
        assert. deepStrictEqual([...o.write(0xff, 2).buffer], [0xe0, 0, 0, 0, 0, 0, 0, 0], "2");
        assert. deepStrictEqual([...o.write(0xff, 4).buffer], [0xfe, 0, 0, 0, 0, 0, 0, 0], "4");
        assert. deepStrictEqual([...o.write(0xff, 8).buffer], [0xff, 0xfe, 0, 0, 0, 0, 0, 0], "8");
        assert. deepStrictEqual([...o.write(0, 1).buffer], [0xff, 0xfe, 0, 0, 0, 0, 0, 0], "1 zero");
        assert. deepStrictEqual([...o.write(0xdecafbad, 16).buffer], [0xff, 0xfe, 0xfb, 0xad, 0, 0, 0, 0], "16");
        assert. deepStrictEqual([...o.write(0xdecafbad, 32).buffer.slice(0, 8)], [0xff, 0xfe, 0xfb, 0xad, 0xde, 0xca, 0xfb, 0xad], "32");
        o = new bits.BitOutputStream(8, 4);
        assert. deepStrictEqual([...o.write(0xf00baaf00b, 40).buffer], [0x0f, 0x0, 0xba, 0xaf, 0x00, 0xb0, 0, 0], "40");
    });
    it("writeBit", () => {
        ...
        assert. deepStrictEqual([...o.seek(0).writeBit(0).buffer], [0x6f, 0x80], "seek 0");
        assert. deepStrictEqual([...o.seek(0).writeBit(1).buffer], [0xef, 0x80], "seek 0 1");
        assert. deepStrictEqual([...o.write(0, 4).buffer], [0x87, 0x80], "write 4");
    });
    it("bytes", () => {
        assert. deepStrictEqual([...new bits.BitOutputStream().bytes()], [], "empty");
        assert. deepStrictEqual([...new bits.BitOutputStream(1, 7).bytes()], [0], "7");
        assert. deepStrictEqual([...new bits.BitOutputStream(2, 8).bytes()], [0], "8");
        assert. deepStrictEqual([...new bits.BitOutputStream(2, 9).bytes()], [0, 0], "9");
    });
});
