...
describe("BitInputStream", function () {
  let src = new Uint8Array([0xbe, 0xef, 0xde, 0xca, 0xfb, 0xad, 0xf0, 0x0b, 0xaa]);
  it("iterator", () => {
    assert.equal(
  ...
  it("mapBitfields", () => {
    assert.deepStrictEqual(
      new bits.BitInputStream(src).readFields([4, 8, 1, 3, 44, 5, 7]).map(x => x.toString(16)),
