...
test("parent", () => {
	expect(h.parent(0)).toBeUndefined();
	expect(h.parent(1)).toBe(-1);
	expect(h.parent(2)).toBe(-1);
	expect(h.parent(3)).toBe(-1);
	expect(h.parent(4)).toBe(-1);
	expect(h.parent(5)).toBe(2);
	expect(h.parent(6)).toBe(2);
	expect(h.parent(7)).toBe(2);
	expect(h.parent(8)).toBe(2);
});

test("children", () => {
	expect(h.children(0)).toEqual([2, 10, 20, 15]);
	expect(h.children(1)).toEqual([18, 23, 22, 5]);
	expect(h.children(2)).toBeUndefined();
});
