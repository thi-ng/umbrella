...
export const mixBilinear =
    (a: number, b: number, c: number, d: number, u: number, v: number) =>
        mix(mix(a, b, u), mix(c, d, u), v);

export const mixQuadratic =
    (a: number, b: number, c: number, t: number) => {
        const s = 1 - t;
...
export const tween =
    (f: (t: number) => number, from: number, to: number) =>
...
export const decimated =
    (n: number, t: number) =>
        Math.floor(t * n) / n;

export const bounce =
    (k: number, amp: number, t: number) => {
        const tk = t * k;
...
export const cubicPulse =
    (w: number, c: number, t: number) => {
        t = Math.abs(t - c);
        return t > w ?
            0 :
            (t /= w, 1 - t * t * (3 - 2 * t));
