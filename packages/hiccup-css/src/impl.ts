...
const withScope =
    (xf: Transducer<any, any>, scope: string) =>
        comp(xf, map((x) => isString(x) && x.indexOf(" .") == 0 ? x + scope : x));

export const expand =
    (acc: string[], parent: any[], rules: any[], opts: CSSOpts) => {
...
const formatRule =
    (parent: any[], sel: any[], curr: any, opts: CSSOpts) => {
        const f = opts.format;
...
export const formatDecls =
    (rules: any, opts: CSSOpts) => {
        const f = opts.format;
