 ...
 * values associated to each entry. {@link ICache.has} will only return
 * true and {@link ICache.get} only returns a cached value if its TTL
 * hasn't yet expired. When adding a new value to the cache, first
 * removes expired entries and if still not sufficient space then
 * removes entries in LRU order. {@link ICache.set} takes an optional
 * entry specific `ttl` arg. If not given, uses the cache instance's
 * default (provided via ctor option arg). If no instance TTL is given,
