...
describe("transducers", () => {
    ...
    it("works combined", (done) => {
        src.subscribe(
            ...
            tx.comp(tx.map((x: number) => x * 10), tx.map((x: number) => x + 1))
        );
    });

    it("does early termination", (done) => {
        src.subscribe(check([data[0], data[1]], done), tx.take(2));
    });

    it("emits multiple values", (done) => {
        src.subscribe(
            check([10, 10, 20], done),
            tx.comp(tx.mapcat((x) => [x, x]), tx.take(3))
        );
    });
    ...
    it("emits remaining", (done) => {
        src.subscribe(check([[10, 20], [30]], done), tx.partition(2, true));
    });
});
