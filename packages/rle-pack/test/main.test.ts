...
import { decodeBinary, encodeBinary } from "../src/index.js";
...
test("3bit", () => {
	let packed = encodeBinary(src1k, src1k.length, 3);
	expect([...packed]).toEqual([
		0, 0, 4, 0, 17, 27, 255, 1, 255, 18, 24, 212, 78, 24, 5, 134, 68, 227,
		82, 30, 3, 196, 0,
	]);
	let dest = decodeBinary(packed);
	expect(dest).toEqual(src1k);

	packed = encodeBinary(src1k, src1k.length, 3, [1, 2, 4, 9]);
	expect([...packed]).toEqual([
		0, 0, 4, 0, 16, 9, 199, 255, 140, 134, 234, 206, 96, 89, 150, 119, 89,
		15, 241, 0,
	]);
	dest = decodeBinary(packed);
	expect(dest).toEqual(src1k);
});
