...
const testFromABGR = (fmt: IntFormat, specs: number[]) => {
	expect(fmt.fromABGR(0xff000000) >>> 0).toBe(specs[0]);
	expect(fmt.fromABGR(0xffff0000) >>> 0).toBe(specs[1]);
	expect(fmt.fromABGR(0xff00ff00) >>> 0).toBe(specs[2]);
	expect(fmt.fromABGR(0xff0000ff) >>> 0).toBe(specs[3]);
};

const testToABGR = (fmt: IntFormat, specs: number[]) => {
	expect(fmt.toABGR(specs[0]) >>> 0).toBe(0xff000000);
	expect(fmt.toABGR(specs[1]) >>> 0).toBe(0xffff0000);
	expect(fmt.toABGR(specs[2]) >>> 0).toBe(0xff00ff00);
	expect(fmt.toABGR(specs[3]) >>> 0).toBe(0xff0000ff);
};
...
test("ABGR8888", () => {
	testFromABGR(ABGR8888, [0xff000000, 0xffff0000, 0xff00ff00, 0xff0000ff]);
});
