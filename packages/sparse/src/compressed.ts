...
export const insert =
    (id1: number, id2: number, id1max: number, x: number, idx: number, major: number[], minor: number[], data: number[]) => {
...
export const remove =
    (id1: number, id1max: number, idx: number, major: number[], minor: number[], data: number[]) => {
        data.splice(idx, 1);
        minor.splice(idx, 1);
        for (let i = id1 + 1; i <= id1max; i++) {
            major[i]--;
