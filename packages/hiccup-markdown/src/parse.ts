...
const transition =
    (ctx: FSMCtx, id: State): ParseResult => (
        ctx.children = [],
        ctx.body = "",
        [id]
    );
...
const collectList = (
    ...
    (ctx): ParseResult => (
        collectLi(ctx, item),
        [State.START, [list(type, ctx.container)]]
    );
    ...
    return fsm<string, FSMCtx, any[]>(
        {
            [State.START]:
                alts(
                    [
                        ...
                        seq(
                            [repeat(str(HR), 3, Infinity), str(NL)],
                            () => [State.START, [tags.hr()]]
                        ),
            ...
            [State.PARA]:
                matchPara(State.PARA, State.END_PARA),
            ...
            [State.BLOCKQUOTE]:
                matchPara(State.BLOCKQUOTE, State.END_BLOCKQUOTE),
            ...
            [State.HEADING]:
                matchPara(State.HEADING, State.END_HEADING),
            ...
            [State.END_LI]:
                alts(
                    [
                        ...
                        str(LI, (ctx) => (collectLi(ctx, tags.li), transition(ctx, State.LI)))
                    ],
                    collect(State.LI)
                ),
