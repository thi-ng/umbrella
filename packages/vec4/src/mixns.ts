 ...
 */
export const mixNS4: VecOpSVVN = (o,a,b,n,io=0,ia=0,ib=0,so=1,sa=1,sb=1) => {
!o && (o=a);o[io]=a[ia]+(b[ib]-a[ia])*n;o[io+so]=a[ia+sa]+(b[ib+sb]-a[ia+sa])*n;o[io+2*so]=a[ia+2*sa]+(b[ib+2*sb]-a[ia+2*sa])*n;o[io+3*so]=a[ia+3*sa]+(b[ib+3*sb]-a[ia+3*sa])*n;return o;
};