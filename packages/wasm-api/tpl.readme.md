...
On the TypeScript/JS side, the memory-mapped wrappers (e.g. `$Foo` and `$Bar`) can be used in combination with the `WasmBridge` to obtain fully typed views (according to the generated types) of the underlying WASM memory. Basic use is like:
