...
const randomSeq =
    (num, prob = 0.5) => [...repeatedly(() => Math.random() < prob ? 1 : 0, num)];

const randomizeGrid =
    (prob = 0.5) => (grid = randomSeq(W * H, prob));
...
export const convolve =
    (src: number[], rules: number[], width: number, height: number, rstride = 9, wrap = true) =>
        transduce(
            comp(
                multiplex(
...
const format =
    (src: number[], width: number, fill = "\u2588", empty = " ") =>
        transduce(
            comp(
...
const setRule =
    (i: number, j: number, s: number, rstride = 9) => {
...
const checkbox =
    (x, onchange) => ["input", { type: "checkbox", checked: !!x, onchange }];
...
const isPreset =
    (id) => presets.findIndex((x) => x[0] === id) !== -1;
